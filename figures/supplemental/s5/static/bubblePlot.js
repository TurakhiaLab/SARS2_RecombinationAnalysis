...
async function bubblePlot(svg, config) {
  ...
  // Data
  const data = await d3.csv(config["pangoClustersData"]);
  const statData = await d3.csv(config["monthlyFitnessData"]);
  ...
  let y = d3
    .scaleLinear()
    .domain([minScore - AXIS_PADDING, maxScore + AXIS_PADDING])
    .range([height, 0]);
